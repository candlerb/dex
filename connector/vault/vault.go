...
func (c *Config) Open(id string, logger log.Logger) (conn connector.Connector, err error) {
	...
	for claim, _ := range c.ClaimMappings {
	...
	case "oidc":
		return &oidcConnector{vaultConnector{*c, logger, httpClient}}, nil
	case "":
...
func (c *oidcConnector) LoginURL(s connector.Scopes, callbackURL, state string) (string, []byte, error) {
	...
	if rx.Data.AuthURL == "" {
		...
		return "", nil, fmt.Errorf("Vault returned empty auth_url.  Did you include %s in allowed_redirect_uris?", callbackURL)
	}
	...
	if err != nil {
		return "", nil, fmt.Errorf("Unable to parse redirect_uri")
	}
	q := redirectURL.Query()
	...
	return redirectURL.String(), []byte(vaultState), nil
}
